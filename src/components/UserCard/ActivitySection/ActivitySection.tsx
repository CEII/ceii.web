...
const ActivitySection: FunctionComponent<UserCardProps> = ({ isPair }) => {
    return (
        <div className="flex w-full justify-between mt-5 md:mt-12">
            <span className="font-bold text-[0.830rem] md:text-[0.95rem] lg:text-[1.1rem] xl:text-lg"> Actividad </span>
            <Menu
                as="div"
                className={`${
            ...
            >
                <Menu.Button className=" h-max px-2 py-1 text-[0.6rem] flex justify-between items-center font-semibold md:text-[0.75rem] xl:text-[0.85rem]">
                    {/* Aca ira el rol actual del usuario */}
                    Actividad
                    <ChevronDownIcon className="w-4" />
                </Menu.Button>
                <Menu.Items className="flex flex-col w-full text-[0.6rem] rounded-xl p-1 space-y-1 font-semibold break-words md:text-[0.75rem] xl:text-[0.85rem]">
                    <Menu.Item>
